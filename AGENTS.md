...
- Bejcapital repo: `/Users/morpheus/berkode/bejcapital`; `terminal.cwd` for bej/ops/social profiles points there; after secret edits run `bash ~/.hermes/scripts/bejcapital-propagate-env.sh --encrypt` then `python3 ~/.hermes/scripts/sync-keys-from-bejcapital.py`. Propagate platforms: `agency`, `bejtrader`, `predictx`, `bejmind`, `bejresearch`, `hermes` (`# platforms: hermes` is enough for Hermes + social sync — no separate `hermes_social` tag). External submodules: `camofox-browser` (berkode fork; fleet service + `CAMOFOX_URL`), `tribev2` at `external/tribev2` (was `bejsentiment/third_party/tribev2`; default `TRIBEV2_REPO_PATH=../external/tribev2`).
