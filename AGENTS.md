...
- Keep Hermes app code in `~/berkode/hermes-agent` separate from user state in `~/.hermes` — **never delete `~/.hermes`** (profiles, secrets, sessions); never store API keys in the clone `.env` — use `$HERMES_HOME/.env.encrypted` only; update upstream with `git fetch upstream` and `hermes update` without touching profile data.
- On all messaging platforms and CLI, address the user as **Master**; never use their real name in agent replies (email/signatures may use legal name).
- Use lowercase **hermes** as sender/display name on all platforms — not "Hermes" or "Hermes Agent".
- Email and social are draft-only with human approval before send/post; X app **bejhermes** — tag `X_*` with `# platforms: hermes` only; finish with `bash ~/.hermes/scripts/setup-x-social.sh` (OAuth 1.0a + bearer from `profiles/social/.env.encrypted`; Web App OAuth2 PKCE often fails).
...
- Prefer direct, high-clarity agent replies: no flattery or filler; state uncertainty explicitly; challenge weak assumptions; accuracy over agreeability.
- Oracle VM Hermes (`ubuntu@vncash1-int1`): use local terminal backend on the same host as Pimono/bejcapital — not Docker, SSH, or cloud sandboxes; code is rsync-deployed (no `.git`) — sync from Mac via `bejcapital/app/scripts/oracle/sync-mac-oracle-fleet.sh`, not `git pull` on VM.
...
- Bejcapital repo: `/Users/morpheus/berkode/bejcapital`; `terminal.cwd` for bej/ops/social profiles points there; after secret edits run `bash ~/.hermes/scripts/bejcapital-propagate-env.sh --encrypt` then `python3 ~/.hermes/scripts/sync-keys-from-bejcapital.py`. Propagate platforms: `bejcapital` (orchestrator — `bejcapital/config/`; legacy tag `agency` accepted), `bejtrader`, `predictx`, `bejmind`, `bejresearch`, `hermes` (`# platforms: hermes` is enough for Hermes + social sync — no separate `hermes_social` tag). External submodules: `camofox-browser` (berkode fork; fleet service + `CAMOFOX_URL`), `tribev2` at `external/tribev2` (was `bejsentiment/third_party/tribev2`; default `TRIBEV2_REPO_PATH=../external/tribev2`).
- LLM for Hermes: BejMind Pimono on port 3099, OpenAI-compatible proxy on `http://127.0.0.1:5102/v1` (`bejcapital/bejmind`); keys from bejmind encrypted env; Mac profiles use `openrouter/owl-alpha` (`gpt-4o-mini` returns empty via Pimono; keep `max_tokens` ≤300 there); Oracle VM `ubuntu@vncash1-int1` uses local `ollama/gemma3:12b` via the same proxy — keep `ollama/` prefix on custom provider (do not normalize to `gemma3:12b`); `max_tokens` ~2048 for Ollama; Pimono proxy must emit **indexed** `tool_calls` in SSE streams; SSE must use `Connection: close` after `data: [DONE]` or curl/Hermes hang; **`gemma3:12b` has no native Ollama tool support** — `pimono_complete_server.mjs` must use prompt-tools fallback (`<tool_call>{...}</tool_call>`) or Hermes gets empty responses; restart stack with `stop_pimono_stack.sh` (uses `ss`, not `lsof` — missing on Oracle Ubuntu).
- Profile secrets: **runtime reads only** `$HERMES_HOME/.env.encrypted`; never read `.env.encrypt` at runtime. Decrypt requires matching `~/.hermes/config/secret.key` on each host (copy from Mac for VM profiles). Edit master secrets in `bejcapital/config/.env.encrypt` (user keeps and deletes manually). Hermes `profiles/*/.env.encrypt` is staging — auto-removed after encrypt (view/decrypt via `python3 ~/.hermes/scripts/hermes_encryption.py decrypt-count --home …`). Other bejcapital platform `.env.encrypt` may auto-delete after encrypt. Audit: `bash ~/.hermes/scripts/audit-env-staging.sh`. Sync decrypts bejcapital `config/.env.encrypted` only; **X_* → `profiles/social` only**.
- Set `UV_CACHE_DIR=/Users/morpheus/berkode/hermes-agent/.uv-cache` (system `~/.cache/uv` is root-owned on this Mac).
- Hermes venv: Mac uses repo `.venv` (`uv sync`, `source .venv/bin/activate`, `scripts/ensure_hermes_venv.sh` after bare `uv venv`); Oracle VM `ubuntu@vncash1-int1` uses `venv/` with Python 3.13 (`setup-hermes.sh` `PYTHON_VERSION=3.13`); `./hermes` re-execs project venv (`venv/` or `.venv/`). Export `UV_CACHE_DIR` when using uv on Mac.
- Gateway runs manually from default `~/.hermes` via `hermes-services.sh` (profile `cron status` may falsely say gateway down). WhatsApp uses `whatsapp.reply_prefix: "hermes\n"`. Briefs and ops crons deliver to Discord; `bej cron edit <id> --deliver discord` (bare `cron edit` returns "No updates provided").
...
- MASTER operator model: Obsidian vault `~/Master`; live ops under `05-HERMES/` (control-room, company-brain, workflows); versioned templates in repo `docs/MASTER_Second_Brain plan/` — deploy with `hermes master init --vault-path ~/Master [--overwrite]`; MASTER cron `--workdir` loads `HERMES.md`, `terminal.cwd` stays bejcapital for code; 12 employees per `AGILE_REVISION.md`; Kanban `berkode-ops` + `ops` orchestrator; `install-cron` jobs start paused; shadow mode before enabling cron; no autonomous live trading from Hermes.
