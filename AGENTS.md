...
- Keep Hermes app code in the git clone separate from personal/business state in `~/.hermes`; never store API keys in the clone `.env` — use `$HERMES_HOME/.env.encrypted` only; update upstream with `git fetch upstream` and `hermes update` without touching profile data.
...
- Oracle VM Hermes (`ubuntu@vncash1-int1`): use local terminal backend on the same host as Pimono/bejcapital — not Docker, SSH, or cloud sandboxes for normal bej ops.
...
- Bejcapital repo: `/Users/morpheus/berkode/bejcapital`; `terminal.cwd` for bej/ops/social profiles points there; after secret edits run `bash ~/.hermes/scripts/bejcapital-propagate-env.sh --encrypt` then `python3 ~/.hermes/scripts/sync-keys-from-bejcapital.py`. Propagate platforms: `agency`, `bejtrader`, `predictx`, `bejmind`, `bejresearch`, `hermes` (`# platforms: hermes` is enough for Hermes + social sync — no separate `hermes_social` tag). Camofox at `external/camofox-browser` (berkode fork of jo-inc/camofox-browser); fleet service `camofox-browser`; BejResearch browser via `CAMOFOX_URL`.
