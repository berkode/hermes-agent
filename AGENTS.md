...
- Profile secrets: **runtime reads only** `$HERMES_HOME/.env.encrypted`; never read `.env.encrypt` at runtime. Edit master secrets in `bejcapital/config/.env.encrypt` (user keeps and deletes manually). Hermes `profiles/*/.env.encrypt` is staging — auto-removed after encrypt (view/decrypt via `python3 ~/.hermes/scripts/hermes_encryption.py decrypt-count --home …`). Other bejcapital platform `.env.encrypt` may auto-delete after encrypt. Audit: `bash ~/.hermes/scripts/audit-env-staging.sh`. Sync decrypts bejcapital `config/.env.encrypted` only; **X_* → `profiles/social` only**.
- Set `UV_CACHE_DIR=/Users/morpheus/berkode/hermes-agent/.uv-cache` (system `~/.cache/uv` is root-owned on this Mac).
- Use the repo `.venv` with `uv` for Hermes CLI (`uv sync`, `source .venv/bin/activate`; export `UV_CACHE_DIR` when using uv).
