...
- LLM for Hermes: BejMind Pimono on port 3099, OpenAI-compatible proxy on `http://127.0.0.1:5102/v1` (`bejcapital/bejmind`); keys from bejmind encrypted env; Mac profiles use `openrouter/owl-alpha` (`gpt-4o-mini` returns empty via Pimono); Oracle VM `ubuntu@vncash1-int1` uses local `ollama/gemma3:12b` — keep `ollama/` prefix on custom provider (do not normalize to `gemma3:12b`); keep `max_tokens` ≤300 on the Pimono path; proxy must forward full OpenAI messages and tools or tool calling breaks (raw longcat XML in responses).
