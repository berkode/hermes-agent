...
### Daemon — auto-push on file save

```bash
python -m bejtrader.tradingview.sync.cli watch
```

Default: **fleet push every 6 hours** (`poll_seconds: 21600`, `push_all_on_poll: true`).
Set `push_all_on_poll: false` for legacy mtime-debounce on save.
...
| `tv-sync.cli watch`                      | Daemon — poll + auto-push. |
| `tv-sync.cli watch --once`               | Single sweep, then exit (CI). |
